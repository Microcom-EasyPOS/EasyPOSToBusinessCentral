...
type
  ...
  private
    ...
    procedure DoSyncronizeMovementsTransactions;
    procedure DoSyncronizeStockRegulationTransactions;
...
begin
  ...
  AddToLog(Format('[INFO] Business Central version: %s (0: Current local based BC witrh basic authentication.   2: BC IN the sky with OAuth2 authentication) ',
    [LF_BC_Version.ToString]));
  ...
  var
    ...
    begin
      try
        ...
        begin
          ReWrite(lExportFile);
          WriteLn(lExportFile, 'EpID' + Delimiter + 'TransID' + Delimiter + 'TransDato' + Delimiter + 'TransTid' + Delimiter + 'Bogf�ringsDato' + Delimiter + 'Bogf�ringsTid' +
            Delimiter + 'Bilagsnummer' + Delimiter + 'Tekst' + Delimiter + 'Type' +
            Delimiter + 'ID' + Delimiter + 'Maskine' + Delimiter + 'Afdeling' + Delimiter + 'Butik' + Delimiter + 'Bel�b');
        end
        else
        begin
          Append(lExportFile);
          ...
          Case QFetchFinancialRecords.FieldByName('POstType').AsInteger of
            ...
            4: // Udbetalinger
              begin
                ...
                // Kontonummer eller debitornummer
                lkmCashstatement.id := Trim(QFetchFinancialRecords.FieldByName('KontoNr').AsString);
                // Vil det v�re muligt at lave en �fedtmule� l�sning, hvor der under eksport af finansposter til BC kunne laves en konvertering fra F til K hvis
            ...
            22: // Tilgodeseddel
              begin
                ...
                begin
                  ...
                  // �Ndret i samarbejde med Berit 19-05-2016 i flg ticket #4952
                  lkmCashstatement.type_ := '0';
                  // Kontonummer eller debitornummer
                  lkmCashstatement.id := Trim(QFetchFinancialRecords.FieldByName('KontoNr').AsString);
                end
...
begin
  (*
    ...
    - Der skabes forbindelse til DB. Fejler den g�res ingenting.
    - Laver BC Object (der sker ingenting her, andet end indstillinger loades)
    - BRUGES IKKE. L�ser (som det er i dag) - hvor mange dage tilbage, skal der kigges efter data.
    - L�ser (som det er i dag) - Hvorn�r k�rte jeg sidst en succesful synk. Det kalder vi lige SidsteDatoForSuccesfuldK�rsel
    - L�ser hvilken afdeling, der skal ligges til grund for kost- og salgspriser
    - Henter data. I dag er det:
    - Varer der er solgt, lagerf�rt eller flyttet i den periode
    - Bruger perioden SidsteDatoForSuccesfuldK�rsel - NU
    - L�ber gennem alle data. Nu inds�ttes der varer via kmItem and kmVariant. Her kan der ske:
    - En vare fejler. Dermed overf�res ingen af denne varianter. Varen kommer p� en fejlliste.
    - Varen overf�res. Alt er godt (faktisk kan jeg se, jeg t�ller op, hvor mange gange, den er overf�rt. Egentligt ubrugeligt)
    - N�r alt er genneml�bet gemmer jeg dato for sidst en succesful synk.
    - Dvs. HVIS der var en fejl, og det var der, s� vil n�ste k�rsel gentage hele det samme igen. Og Robert satte vist bare datoen for sidste k�rsel tli "meget langt tilbage". Og s� g�r rutinen det bare en gang mere.
    - Er der en fejl, sendes der en mail.



    Er der s� varer, som konsekvent fejler, kunner det jo tyde p�, at disse har en anden fejl.
    ...
    begin
      ...
      try
        ...
        try
          ...
          AddToLog(Format('[INFO]   Fetching items. Period %s to %s', [FormatDateTime('yyyy-mm-dd hh:mm:ss', lFromDateAndTime), FormatDateTime('yyyy-mm-dd hh:mm:ss',
            lToDateAndTime)]));
...
{$ENDIF}
          ...
          begin
            ...
            AddToLog('[INFO] Iteration done');
            AddToLog(Format('[INFO]   Exported %d head items, %d head item variants and %d variants', [lExportCounterHeadItems, lExportCounterHeadItemVariants,
              lExportCounterVariants]));
            AddToLog('[INFO] Routine done');
...
procedure TDM.DoSyncronizeMovementsTransactions;
const
      ...
      begin
        ...
        try
          ...
          (*
            ...
            Denne kan s�rge for, at det bare er en INTEGER, der kommer retur.

            Et alternativ er, at �ndre klassen og �ndre det nedenfor.
...
begin
  ...
  try
        ...
        try
          ...
          begin
            ...
            begin
              ...
              LogPerformance('DoSyncronizeMovementsTransactions', lStartTime, lNumberOfExportedMovementsTransactions);
            end
            else
            begin
    ...
    on E: Exception do
    begin
      LogError('DoSyncronizeMovementsTransactions', 'Movements transactions sync failed', E);
      if (tnMain.Active) then
        tnMain.Rollback;
    end;
  end;
...
procedure TDM.DoSyncronizeStockRegulationTransactions;
const
...
begin
  AddToLog('[INFO] DoSyncronizeStockRegulationTransactions - BEGIN');
  lStartTime := NOW;
  if (ConnectToDB) then
  begin
    AddToLog('[INFO] TBusinessCentralSetup.Create');
    ...
    try
      ...
      try
        ...
        // Log
        AddToLog(Format('[INFO]   Fetching stock regulation transactions. Period %s to %s', [FormatDateTime('yyyy-mm-dd hh:mm:ss', lFromDateAndTime),
          FormatDateTime('yyyy-mm-dd hh:mm:ss', lToDateAndTime)]));
        ...
        begin
          // At least 1 record is there - fetch next transactions UD
          BC_TransactionID := FetchNextTransID('stock regulation transactions');
          ...
          begin
            ...
            // This is now done after each succesful transfer inside the loop
            InsertTracingLog(7, lFromDateAndTime, lToDateAndTime, BC_TransactionID);
            LogPerformance('DoSyncronizeStockRegulationTransactions', lStartTime, lNumberOfExportedStockRegulationTransactions);
          end
          else
          begin
            lText := 'An error occurred synchronizing stock regulation transactions to Business Central.' + #13#10 +
              'Attached is a file with information' + #13#10;
            SendErrorMail(LogFileFolder + lStockRegulationsTransactionErrorFileName, 'Stock regulation transactions', lText);
            // Rename error file
            TFile.Move(LogFileFolder + lStockRegulationsTransactionErrorFileName, LogFileFolder + Format('Error_StockRegulationTransactions_%s.txt',
              [FormatDateTime('ddmmyyyy_hhmmss', NOW)]));
            if (tnMain.Active) then
              tnMain.Rollback;
            AddToLog('[ERROR] Export of stock regulation transactions ended with errors.');
        ...
        begin
          if (tnMain.Active) then
            tnMain.Commit;
          AddToLog(Format('[INFO] No stock regulation transactions to export', []));
        end;
      finally
        AddToLog('[INFO] TBusinessCentral - Free');
        FReeAndNil(lBusinessCentral);
...
{$ENDIF}
      ...
      begin
        DoSyncronizeMovementsTransactions;
      end;

      if lSyncronizeStockRegulationsTransactions then
      begin
        DoSyncronizeStockRegulationTransactions;
      end;
